...
def newLinkGanaretor(full_name, email): # this function generate a uniq link and qrcode and return the uniq link, number using the name and the email
    ...
    newNumberId = str(uuid.uuid1().int)[:8] # generate a uniq number id
    newNumberId2 = int(newNumberId)
    qr.add_data({"Name" : full_name,
    "E-mai" : email,
    "Link" :newLink,
    "UniqID" : newNumberId2 }) # add the data(name, email) to the qrcode image
    qr.make(fit=True) # make the qrcode image fit well
    img = qr.make_image(fill_color="black", back_color="white").convert('RGB') # set the background and front color for the qrcode image and convert it to RGB type
    ...
    data = {"newLink" : newLink, "newNumberId" : newNumberId2}
    print(newNumberId)
